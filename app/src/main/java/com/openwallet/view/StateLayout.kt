...
import com.openwallet.view.State.*
...
class StateLayout @JvmOverloads constructor(context: Context, attributeSet: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    var emptyLayoutId = 0
    var errorLayoutId = 0
    var showErrorGoHomeBtn = false
    lateinit var errorGoHomeBtn: View //todo 改为通用的button
    ...
    private fun switchLayout(s: State) {
        ...
        when (state) {
            ...
            Error -> {
                switch(errorView)
                errorGoHomeBtn.visibility = if(showErrorGoHomeBtn) View.VISIBLE else View.GONE
            }
            Content -> {
...
//                    && emptyView?.visibility!=VISIBLE && errorView?.visibility!=VISIBLE)return
    ...
    fun showError(errorMessage: String): StateLayout {
        mHandler.post {
            if (noEmptyAndError) {
                switchLayout(Content)
            } else {
    ...
    var mGoHomeAction: ((errView: View) -> Unit)? = null
...
//            setOnClickListener { retry() }
            ...
            errorGoHomeBtn = findViewById<View>(R.id.btnGoHome).apply {
                setOnClickListener { mGoHomeAction?.invoke(this) }
                visibility = if (showErrorGoHomeBtn) View.VISIBLE else View.GONE
            }
            addView(errorView)
        }
        return this
    ...
    fun config(
        ...
        gohomeAction: ((errView: View) -> Unit)? = null,
    ): StateLayout {
        ...
        if (gohomeAction != null) mGoHomeAction = gohomeAction
        return this
    }
