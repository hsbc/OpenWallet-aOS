...
class CountryCodeListViewModelTest : TestCase() {
    @get:Rule
    ...
    val viewModel = CountryCodeListViewModel(apiRepository)
    ...
    fun testGetCountryCodeInfo() = runBlocking {
        ...
        val response = CountryCodeListResponse(true,"message",data)
        val resultState = ResultState.onAppSuccess(response)

        var expectedResult = MutableLiveData<ResultState<CountryCodeListResponse>>()
        expectedResult.value = resultState

        every { retrofit.create(ApiService::class.java) } returns apiService

        coEvery { apiRepository.getCountryCodeInfo() } coAnswers { response }

        val actualResult = viewModel.getCountryCodeInfo()
